# create-plan preferences

Seeded to `${CLAUDE_PLUGIN_DATA}/preferences/create-plan.md` on first run. The skill reads that copy, not this template, so your values survive plugin updates. `execute-plan` and `create-plan-and-execute` read the same file.

Any field left empty means "use the default documented in SKILL.md."

## plans_dir
<!--
Where HTML plans are written. Absolute, ~/..., or relative to the repo root
...
## open_after_create
<!--
Open the rendered plan with the OS default opener (open / xdg-open) after writing.
